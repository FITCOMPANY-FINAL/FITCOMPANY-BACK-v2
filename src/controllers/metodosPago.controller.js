...
export const crearMetodoPago = async (req, res) => {
  ...
  if (nombreNormalizado.length > 50) {
    return res
      .status(400)
      .json({
        message: "El nombre del método de pago no puede superar 50 caracteres.",
      });
  }
  ...
  try {
    ...
    console.log(
      `✅ Método de pago creado: ${nuevoMetodo.nombre_metodo_pago} (ID: ${nuevoMetodo.id_metodo_pago})`,
    );
...
export const actualizarMetodoPago = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `✅ Método de pago actualizado: ${metodoActualizado.nombre_metodo_pago}`,
    );
...
export const eliminarMetodoPago = async (req, res) => {
  ...
  try {
    ...
    await db("metodos_pago").where("id_metodo_pago", id).del();

    console.log(
      `✅ Método de pago eliminado (eliminación física): ${metodo.nombre_metodo_pago} (ID: ${id})`,
    );
    ...
    if (error.code === "23503") {
      return res.status(409).json({
        message:
          "No se puede eliminar el método de pago porque está siendo usado en ventas.",
      });
    }
