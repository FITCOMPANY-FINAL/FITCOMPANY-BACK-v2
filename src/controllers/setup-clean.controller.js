...
export const cleanAndSetup = async (req, res) => {
  try {
    ...
    const formularios = [
      ...
      { titulo: "Ventas", url: "/ventas", es_padre: false, orden: 3, padre: "Operaciones" },
      { titulo: "Compras", url: "/compras", es_padre: false, orden: 4, padre: "Operaciones" },
      { titulo: "Métodos de Pago", url: "/metodos-pago", es_padre: false, orden: 5, padre: "Operaciones" },

      // Inventario (PADRE)
      { titulo: "Inventario", url: "/inventario", es_padre: true, orden: 6 },
      { titulo: "Categorías", url: "/categorias", es_padre: false, orden: 7, padre: "Inventario" },
      { titulo: "Productos", url: "/productos", es_padre: false, orden: 8, padre: "Inventario" },
      { titulo: "Unidades de Medida", url: "/unidades-medidas", es_padre: false, orden: 9, padre: "Inventario" },

      // Administración (PADRE)
      { titulo: "Administración", url: "/administracion", es_padre: true, orden: 10 },
      { titulo: "Tipos de Identificación", url: "/tipos-identificacion", es_padre: false, orden: 11, padre: "Administración" },
      { titulo: "Roles", url: "/roles", es_padre: false, orden: 12, padre: "Administración" },
      { titulo: "Usuarios", url: "/usuarios", es_padre: false, orden: 13, padre: "Administración" },
      { titulo: "Formularios", url: "/formularios", es_padre: false, orden: 14, padre: "Administración" },
      { titulo: "Permisos", url: "/permisos", es_padre: false, orden: 15, padre: "Administración" },

      // Seguridad (PADRE)
      { titulo: "Seguridad", url: "/seguridad", es_padre: true, orden: 16 },
      { titulo: "Gestión de Usuarios", url: "/seguridad/usuarios", es_padre: false, orden: 17, padre: "Seguridad" },

      // Reportes (PADRE)
      { titulo: "Reportes", url: "/reportes", es_padre: true, orden: 18 },
      { titulo: "Reporte de Ventas", url: "/reportes/ventas", es_padre: false, orden: 19, padre: "Reportes" },
      { titulo: "Reporte de Compras", url: "/reportes/compras", es_padre: false, orden: 20, padre: "Reportes" },
      { titulo: "Reporte de Inventario", url: "/reportes/inventario", es_padre: false, orden: 21, padre: "Reportes" },
    ];
    ...
    for (const form of formularios) {
      try {
        ...
        const result = await db("formularios").insert({
        ...
        console.log(`✅ ${form.titulo}${padreId ? ` (hijo de ${form.padre})` : ' (PADRE)'}`);
        creados++;
      } catch (error) {
    ...
    const todosLosFormularios = await db("formularios").select("id_formulario");
